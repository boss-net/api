...
from configs.app_config import BossConfig


def test_boss_config(monkeypatch):
    ...
    monkeypatch.setenv("DB_DATABASE", "boss")
...
def test_flask_configs(monkeypatch):
    ...
    flask_app.config.from_mapping(BossConfig().model_dump())  # pyright: ignore
    ...
    assert config["SQLALCHEMY_DATABASE_URI"] == "postgresql://postgres:postgres@localhost:5432/boss"
