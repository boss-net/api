...
import (
	...
	"github.com/boss-net/api/boss-plugin/cmd/commandline/plugin"
...
If no parameters are provided, an interactive mode will be started.`,
	...
	pluginChecksumCommand = &cobra.Command{
		...
		Short: "Checksum",
		Long:  "Calculate the checksum of the plugin, you need specify the plugin path or .bosspkg file path",
		Args:  cobra.ExactArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			pluginPath := args[0]
...
func init() {
  ...
  - python: Python language`)
	pluginInitCommand.Flags().StringVar(&minBossVersion, "min-boss-version", "", "Minimum Boss version required")
