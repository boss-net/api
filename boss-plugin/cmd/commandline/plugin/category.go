...
const PLUGIN_GUIDE = `Before starting, here's some basic knowledge about Plugin types in Boss:
...
` + BOLD + `- Endpoint` + RESET + `: ` + GREEN + `Similar to Service API in Boss and Ingress in Kubernetes. Extend HTTP services as endpoints with custom logic.` + RESET + `
