...
import (
	...
	"github.com/boss-net/api/boss-plugin/pkg/entities/constants"
...
func InitPluginWithFlags(
	...
	// Set minimal Boss version if provided
	if minBossVersion != "" {
		ver := m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire)
		ver.SetMinimalBossVersion(minBossVersion)
...
func (m model) createPlugin() {
	...
	if m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire).MinimalBossVersion() != "" {
		manifest.Meta.MinimumBossVersion = parser.ToPtr(m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire).MinimalBossVersion())
	}
