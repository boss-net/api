## User Guide of how to develop a Boss Plugin
...
Before start, you need some basic knowledge about the Plugin types, Plugin supports to extend the following abilities in Boss:
...
- **Endpoint**: Like Service API in Boss and Ingress in Kubernetes, you can extend a http service as an endpoint and control its logics using your own code.
...
I believe you have chosen the right type for your Plugin while creating it, if not, you can change it later by mamuning the `manifest.yaml` file.
...
- plugins(object, required)：Plugin extension specific ability yaml file list, absolute path in the plugin package, if you need to extend the model, you need to define a file like openai.yaml, and fill in the path here, and the file on the path must exist, otherwise the packaging will fail.
  - Format
    - tools(list[string]): Extended tool suppliers, as for the detailed format, please refer to [Tool Guide](https://docs.boss.ai/plugins/schema-definition/tool)
    - models(list[string])：Extended model suppliers, as for the detailed format, please refer to [Model Guide](https://docs.boss.ai/plugins/schema-definition/model)
    - endpoints(list[string])：Extended Endpoints suppliers, as for the detailed format, please refer to [Endpoint Guide](https://docs.boss.ai/plugins/schema-definition/endpoint)
  - Restrictions
...
- [OpenAI](https://github.com/boss-net/boss-plugin-sdks/tree/main/python/examples/openai): best practice for model provider
- [Google Search](https://github.com/boss-net/boss-plugin-sdks/tree/main/python/examples/google): a simple example for tool provider
- [Neko](https://github.com/boss-net/boss-plugin-sdks/tree/main/python/examples/neko): a funny example for endpoint group
...
- `INSTALL_METHOD`: Set this to `remote`, your plugin will connect to a Boss instance through the network.
- `REMOTE_INSTALL_URL`: The URL of debugging host and port of plugin-daemon service from your Boss instance, eg. `debug.boss.ai:5003`. Either the [Boss SaaS](https://debug.boss.ai) or [self-hosted Boss instance](https://docs.boss.ai/en/getting-started/install-self-hosted/readme) can be used.
- `REMOTE_INSTALL_KEY`: You should get your debugging key from the Boss instance you used, at the right top of the plugin management page, you can see a button with a `debug` icon, click it and you will get the key.
...
Refresh the page of your Boss instance, you should be able to see your Plugin in the list now, but it will be marked as `debugging`, you can use it normally, but not recommended for production.

### Publish and Update the Plugin

To streamline your plugin update workflow, you can configure GitHub Actions to automatically create PRs to the Boss plugin repository whenever you create a release.

##### Prerequisites

- Your plugin source repository
- A fork of the boss-plugins repository
...
For detailed setup instructions and example configuration, visit: [GitHub Actions Workflow Documentation](https://docs.boss.ai/plugins/publish-plugins/plugin-auto-publish-pr)
...
```bash
boss-plugin plugin package ./ROOT_DIRECTORY_OF_YOUR_PLUGIN
```

you will get a `plugin.bosspkg` file, that's all, you can submit it to the Marketplace now, look forward to your Plugin being listed!
