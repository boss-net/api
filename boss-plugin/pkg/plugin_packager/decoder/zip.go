...
import (
	...
	"github.com/boss-net/api/boss-plugin/pkg/plugin_packager/consts"
)
...
func newZipPluginDecoder(
	...
	if err != nil {
		return nil, errors.New(strings.ReplaceAll(err.Error(), "zip", "bosspkg"))
	}
...
func (z *ZipPluginDecoder) Verification() (*Verification, error) {
	...
	// it's historical reason that all plugins are signed by boss-net
	return nil, nil
}
...
func (z *ZipPluginDecoder) Assets() (map[string][]byte, error) {
	// FIXES: https://github.com/boss-net/api/boss-plugin/issues/166
