...
import (
	...
	"github.com/boss-net/api/boss-plugin/pkg/plugin_packager/packager"
...
//go:embed testdata/.bossignore
var boss_ignore []byte
...
func TestPackagerAndVerifier(t *testing.T) {
	...
	// create .bossignore
	if err := os.WriteFile(filepath.Join(tempDir, ".bossignore"), boss_ignore, 0644); err != nil {
		t.Errorf("failed to write .bossignore: %s", err.Error())
		return
	}
...
func TestWrongSign(t *testing.T) {
	...
	// mamun the signed file, signature is at the end of the file
