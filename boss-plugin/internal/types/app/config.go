...
type Config struct {
	...
	// boss inner api
	BossInnerApiURL string `envconfig:"BOSS_INNER_API_URL" validate:"required"`
	BossInnerApiKey string `envconfig:"BOSS_INNER_API_KEY" validate:"required"`

	// storage config
	// https://github.com/boss-net/api/cloud/blob/main/oss/factory/factory.go
	...
	BossInvocationConnectionIdleTimeout int `envconfig:"BOSS_INVOCATION_CONNECTION_IDLE_TIMEOUT" validate:"required"`

	BossPluginServerlessConnectorURL           *string `envconfig:"BOSS_PLUGIN_SERVERLESS_CONNECTOR_URL"`
	BossPluginServerlessConnectorAPIKey        *string `envconfig:"BOSS_PLUGIN_SERVERLESS_CONNECTOR_API_KEY"`
	BossPluginServerlessConnectorLaunchTimeout int     `envconfig:"BOSS_PLUGIN_SERVERLESS_CONNECTOR_LAUNCH_TIMEOUT"`
	...
	// boss invocation write timeout in milliseconds
	BossInvocationWriteTimeout int64 `envconfig:"BOSS_BACKWARDS_INVOCATION_WRITE_TIMEOUT" default:"5000"`
	// boss invocation read timeout in milliseconds
	BossInvocationReadTimeout int64 `envconfig:"BOSS_BACKWARDS_INVOCATION_READ_TIMEOUT" default:"240000"`
}
...
func (c *Config) Validate() error {
	...
	if c.Platform == PLATFORM_SERVERLESS {
		if c.BossPluginServerlessConnectorURL == nil {
			return fmt.Errorf("boss plugin serverless connector url is empty")
		}

		if c.BossPluginServerlessConnectorAPIKey == nil {
			return fmt.Errorf("boss plugin serverless connector api key is empty")
		}
