...
import (
	...
	"github.com/boss-net/api/boss-plugin/internal/core/boss_invocation/tester"
...
func getTestSession() *session_manager.Session {
	return session_manager.NewSession(
		session_manager.NewSessionPayload{
			...
			Declaration:            nil,
			BackwardsInvocation:    tester.NewMockedBossInvocation(),
			IgnoreCache:            true,
		},
	)
}
...
func TestBackwardsInvocationAllPermittedPermission(t *testing.T) {
	...
	invokeTextEmbeddingRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_TEXT_EMBEDDING, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeTextEmbeddingRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeRerankRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_RERANK, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeRerankRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeTtsRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_TTS, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeTtsRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeSpeech2textRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_SPEECH2TEXT, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeSpeech2textRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeModerationRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_MODERATION, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeModerationRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeToolRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_TOOL, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeToolRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeNodeParameterExtractorRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_NODE_PARAMETER_EXTRACTOR, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeNodeParameterExtractorRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeNodeQuestionClassifierRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_NODE_QUESTION_CLASSIFIER, "", getTestSession(), nil, nil)
	if err := checkPermission(&allPermittedRuntime, invokeNodeQuestionClassifierRequest); err != nil {
		t.Errorf("checkPermission failed: %s", err.Error())
	}

	invokeAppRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_APP, "", getTestSession(), nil, nil)
...
func TestBackwardsInvocationAllDeniedPermission(t *testing.T) {
	...
	invokeLlmRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_LLM, "", getTestSession(), nil, nil)
	...
	invokeNodeRequest := NewBackwardsInvocation(boss_invocation.INVOKE_TYPE_NODE_PARAMETER_EXTRACTOR, "", getTestSession(), nil, nil)
