...
import (
	...
	"github.com/boss-net/api/boss-plugin/internal/core/plugin_daemon/backwards_invocation"
	"github.com/boss-net/api/boss-plugin/internal/core/plugin_daemon/backwards_invocation/transaction"
	"github.com/boss-net/api/boss-plugin/internal/core/plugin_daemon/generic_invoke"
...
func GenericInvokePlugin[Req any, Rsp any](
	...
	listener.Listen(func(chunk plugin_entities.SessionMessage) {
		...
		case plugin_entities.SESSION_MESSAGE_TYPE_INVOKE:
			...
			if err := backwards_invocation.InvokeBoss(
				...
				response.WriteError(errors.New(parser.MarshalJson(map[string]string{
					"error_type": "invoke_boss_error",
					"message":    fmt.Sprintf("invoke boss failed: %s", err.Error()),
				})))
				response.Close()
				return
			}
