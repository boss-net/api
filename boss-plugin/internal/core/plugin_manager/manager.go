...
import (
	...
	"github.com/boss-net/api/boss-plugin/internal/core/boss_invocation/real"
	"github.com/boss-net/api/boss-plugin/internal/core/plugin_manager/debugging_runtime"
	"github.com/boss-net/api/boss-plugin/internal/core/plugin_manager/media_transport"
	serverless "github.com/boss-net/api/boss-plugin/internal/core/plugin_manager/serverless_connector"
	...
	"github.com/boss-net/api/boss-plugin/internal/utils/cache"
	"github.com/boss-net/api/boss-plugin/internal/utils/cache/helper"
	"github.com/boss-net/api/boss-plugin/internal/utils/lock"
...
type PluginManager struct {
	...
	// backwardsInvocation is a handle to invoke boss
	backwardsInvocation boss_invocation.BackwardsInvocation
...
func (p *PluginManager) Launch(configuration *app.Config) {
	...
	invocation, err := real.NewBossInvocationDaemon(
		real.NewBossInvocationDaemonPayload{
			BaseUrl:      configuration.BossInnerApiURL,
			CallingKey:   configuration.BossInnerApiKey,
			WriteTimeout: configuration.BossInvocationWriteTimeout,
			ReadTimeout:  configuration.BossInvocationReadTimeout,
		},
	)
	if err != nil {
		log.Panic("init boss invocation daemon failed: %s", err.Error())
	}
...
func (p *PluginManager) BackwardsInvocation() boss_invocation.BackwardsInvocation {
