...
func (p *PluginManager) InstallToAWSFromPkg(
	...
	*stream.Stream[PluginInstallResponse], error,
	...
	response, err := serverless.LaunchPlugin(originalPackager, decoder, p.config.BossPluginServerlessConnectorLaunchTimeout, false)
	if err != nil {
		return nil, err
	}
...
func (p *PluginManager) ReinstallToAWSFromPkg(
	...
	*stream.Stream[PluginInstallResponse], error,
	...
	response, err := serverless.LaunchPlugin(
		originalPackager,
		decoder,
		p.config.BossPluginServerlessConnectorLaunchTimeout,
