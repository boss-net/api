...
func (s *BossServer) OnBoot(c gnet.Engine) (action gnet.Action) {
	s.engine = c
	return gnet.None
}

func (s *BossServer) OnOpen(c gnet.Conn) (out []byte, action gnet.Action) {
...
func (s *BossServer) OnClose(c gnet.Conn, err error) (action gnet.Action) {
	// plugin disconnected
	s.pluginsLock.Lock()
...
func (s *BossServer) OnShutdown(c gnet.Engine) {
	close(s.shutdownChan)
}

func (s *BossServer) OnTraffic(c gnet.Conn) (action gnet.Action) {
...
func (s *BossServer) onMessage(runtime *RemotePluginRuntime, message []byte) {
	// handle message
