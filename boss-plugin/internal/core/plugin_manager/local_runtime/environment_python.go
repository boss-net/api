...
func (p *LocalPluginRuntime) InitPythonEnvironment() error {
	...
	if _, err := os.Stat(path.Join(p.State.WorkingPath, ".venv")); err == nil {
		// check if venv is valid, try to find .venv/boss/plugin.json
		if _, err := os.Stat(path.Join(p.State.WorkingPath, ".venv/boss/plugin.json")); err != nil {
			...
			// https://github.com/boss-net/boss-plugin-sdks/commit/161045b65f708d8ef0837da24440ab3872821b3b
	...
	defer func() {
		...
		if !success {
			...
			// create boss/plugin.json
			pluginJsonPath := path.Join(p.State.WorkingPath, ".venv/boss/plugin.json")
...
func (p *LocalPluginRuntime) patchPluginSdk(requirementsPath string) error {
	...
	if pluginSdkVersionObj.LessThan(version.Must(version.NewVersion("0.0.1b70"))) {
		// get boss-plugin path
		command := exec.Command(p.pythonInterpreterPath, "-c", "import importlib.util;print(importlib.util.find_spec('boss_plugin').origin)")
...
func (p *LocalPluginRuntime) getPluginSdkVersion(requirements string) (string, error) {
	...
	re := regexp.MustCompile(`(?:boss[_-]plugin)(?:~=|==)([0-9.a-z]+)`)
	...
	// Extract all version constraints for boss-plugin
	// Try to match version ranges with multiple constraints
	// For example: boss-plugin>=0.1.0,<0.2.0
	reAllConstraints := regexp.MustCompile(`(?:boss[_-]plugin)([><]=?|==)([0-9.a-z]+)(?:,([><]=?|==)([0-9.a-z]+))?`)
