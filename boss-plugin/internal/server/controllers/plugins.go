...
func UploadPlugin(app *app.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		bossPkgFileHeader, err := c.FormFile("boss_pkg")
		if err != nil {
		...
		if bossPkgFileHeader.Size > app.MaxPluginPackageSize {
		...
		bossPkgFile, err := bossPkgFileHeader.Open()
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		defer bossPkgFile.Close()

		c.JSON(http.StatusOK, service.UploadPluginPkg(app, c, tenantId, bossPkgFile, verifySignature))
	}
}

func UploadBundle(app *app.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		bossBundleFileHeader, err := c.FormFile("boss_bundle")
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		...
		if bossBundleFileHeader.Size > app.MaxBundlePackageSize {
			c.JSON(http.StatusOK, exception.BadRequestError(errors.New("file size exceeds the maximum limit")).ToResponse())
			return
		}
		...
		bossBundleFile, err := bossBundleFileHeader.Open()
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		defer bossBundleFile.Close()

		c.JSON(http.StatusOK, service.UploadPluginBundle(app, c, tenantId, bossBundleFile, verifySignature))
	}
}
