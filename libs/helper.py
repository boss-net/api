...
def length_prefixed_response(magic_number: int, response: Union[Mapping, Generator, RateLimitGenerator]) -> Response:
    ...
    For a compatibility with latest plugin daemon https://github.com/boss-net/boss-plugin-daemon/pull/341
...
class TokenManager:
    ...
    def generate_token(
        ...
        expiry_minutes = boss_config.model_dump().get(f"{token_type.upper()}_TOKEN_EXPIRY_MINUTES")
