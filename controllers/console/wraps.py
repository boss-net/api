...
def only_edition_enterprise(view):
    @wraps(view)
    def decorated(*args, **kwargs):
        if not boss_config.ENTERPRISE_ENABLED:
            abort(404)
...
def only_edition_self_hosted(view):
    @wraps(view)
    def decorated(*args, **kwargs):
        if boss_config.EDITION != "SELF_HOSTED":
            abort(404)
            ...
            if features.billing.enabled:
                if resource == "add_segment":
                    if features.billing.subscription.plan == "sandbox":
                        abort(
                            403,
                            "To unlock this feature and elevate your Boss experience, please upgrade to a paid plan.",
                        )
                else:
                    return view(*args, **kwargs)
...
def setup_required(view):
    ...
    def decorated(*args, **kwargs):
        # check setup
        if (
            boss_config.EDITION == "SELF_HOSTED"
            and os.environ.get("INIT_PASSWORD")
            and not db.session.query(BossSetup).first()
        ):
            raise NotInitValidateError()
        elif boss_config.EDITION == "SELF_HOSTED" and not db.session.query(BossSetup).first():
            raise NotSetupError()
