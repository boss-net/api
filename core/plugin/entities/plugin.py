...
class PluginDeclaration(BaseModel):
    ...
    class Meta(BaseModel):
        minimum_boss_version: Optional[str] = Field(default=None, pattern=r"^\d{1,4}(\.\d{1,4}){1,3}(-\w{1,16})?$")
...
class GenericProviderID:
    ...
    def __init__(self, value: str, is_hardcoded: bool = False) -> None:
        ...
        if not re.match(r"^[a-z0-9_-]+\/[a-z0-9_-]+\/[a-z0-9_-]+$", value):
            # check if matches [a-z0-9_-]+, if yes, append with boss-net/$value/$value
            if re.match(r"^[a-z0-9_-]+$", value):
                value = f"boss-net/{value}/{value}"
            else:
    ...
    def is_boss_net(self) -> bool:
        return self.organization == "boss-net"
...
class ModelProviderID(GenericProviderID):
    def __init__(self, value: str, is_hardcoded: bool = False) -> None:
        ...
        if self.organization == "boss-net" and self.provider_name == "google":
