...
code_execution_endpoint_url = URL(str(boss_config.CODE_EXECUTION_ENDPOINT))
...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        headers = {"X-Api-Key": boss_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=boss_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=boss_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=boss_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
