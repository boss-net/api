...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=boss_config.RELYT_HOST or "localhost",
                port=boss_config.RELYT_PORT,
                user=boss_config.RELYT_USER or "",
                password=boss_config.RELYT_PASSWORD or "",
                database=boss_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
