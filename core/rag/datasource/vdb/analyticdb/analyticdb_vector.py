...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AnalyticdbVector:
        ...
        if boss_config.ANALYTICDB_HOST is None:
            # implemented through OpenAPI
            apiConfig = AnalyticdbVectorOpenAPIConfig(
                access_key_id=boss_config.ANALYTICDB_KEY_ID or "",
                access_key_secret=boss_config.ANALYTICDB_KEY_SECRET or "",
                region_id=boss_config.ANALYTICDB_REGION_ID or "",
                instance_id=boss_config.ANALYTICDB_INSTANCE_ID or "",
                account=boss_config.ANALYTICDB_ACCOUNT or "",
                account_password=boss_config.ANALYTICDB_PASSWORD or "",
                namespace=boss_config.ANALYTICDB_NAMESPACE or "",
                namespace_password=boss_config.ANALYTICDB_NAMESPACE_PASSWORD,
            )
            sqlConfig = None
        else:
            # implemented through sql
            sqlConfig = AnalyticdbVectorBySqlConfig(
                host=boss_config.ANALYTICDB_HOST,
                port=boss_config.ANALYTICDB_PORT,
                account=boss_config.ANALYTICDB_ACCOUNT or "",
                account_password=boss_config.ANALYTICDB_PASSWORD or "",
                min_connection=boss_config.ANALYTICDB_MIN_CONNECTION,
                max_connection=boss_config.ANALYTICDB_MAX_CONNECTION,
                namespace=boss_config.ANALYTICDB_NAMESPACE or "",
            )
            apiConfig = None
