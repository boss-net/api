    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> PGVectoRS:
        ...
        return PGVectoRS(
            ...
            config=PgvectoRSConfig(
                host=boss_config.PGVECTO_RS_HOST or "localhost",
                port=boss_config.PGVECTO_RS_PORT or 5432,
                user=boss_config.PGVECTO_RS_USER or "postgres",
                password=boss_config.PGVECTO_RS_PASSWORD or "",
                database=boss_config.PGVECTO_RS_DATABASE or "postgres",
            ),
            dim=dim,
        )
