...
class MatrixoneVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> MatrixoneVector:
        ...
        config = MatrixoneConfig(
            host=boss_config.MATRIXONE_HOST or "localhost",
            port=boss_config.MATRIXONE_PORT or 6001,
            user=boss_config.MATRIXONE_USER or "dump",
            password=boss_config.MATRIXONE_PASSWORD or "111",
            database=boss_config.MATRIXONE_DATABASE or "boss",
            metric=boss_config.MATRIXONE_METRIC or "l2",
        )
