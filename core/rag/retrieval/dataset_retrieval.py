...
class DatasetRetrieval:
    ...
    def retrieve(
        ...
        boss_documents = [item for item in all_documents if item.provider == "boss"]
        ...
        # deal with boss documents
        if boss_documents:
            records = RetrievalService.format_retrieval_documents(boss_documents)
            if records:
    ...
    def _on_retrieval_end(
        ...
        boss_documents = [document for document in documents if document.provider == "boss"]
        for document in boss_documents:
            if document.metadata is not None:
                dataset_document = (
                    db.session.query(DatasetDocument)
    ...
    def _fetch_model_config(
        ...
        if provider_model.status == ModelStatus.NO_CONFIGURE:
            ...
            raise ValueError(f"Boss Hosted OpenAI {model_name} currently not support.")
