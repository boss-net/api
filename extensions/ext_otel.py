...
def init_app(app: BossApp):
    ...
    def init_flask_instrumentor(app: BossApp):
        meter = get_meter("http_metrics", version=boss_config.CURRENT_VERSION)
    ...
    def init_sqlalchemy_instrumentor(app: BossApp):
    ...
    class ExceptionLoggingHandler(logging.Handler):
        ...
        def emit(self, record: logging.LogRecord):
            try:
                if record.exc_info:
                    tracer = get_tracer_provider().get_tracer("boss.exception.logging")
    ...
    resource = Resource(
        attributes={
            ResourceAttributes.SERVICE_NAME: boss_config.APPLICATION_NAME,
            ResourceAttributes.SERVICE_VERSION: f"boss-{boss_config.CURRENT_VERSION}-{boss_config.COMMIT_SHA}",
            ResourceAttributes.PROCESS_PID: os.getpid(),
            ResourceAttributes.DEPLOYMENT_ENVIRONMENT: f"{boss_config.DEPLOY_ENV}-{boss_config.EDITION}",
            ResourceAttributes.HOST_NAME: socket.gethostname(),
            ResourceAttributes.HOST_ARCH: platform.machine(),
            "custom.deployment.git_commit": boss_config.COMMIT_SHA,
    ...
    sampler = ParentBasedTraceIdRatio(boss_config.OTEL_SAMPLING_RATE)
    ...
    protocol = (boss_config.OTEL_EXPORTER_OTLP_PROTOCOL or "").lower()
    if boss_config.OTEL_EXPORTER_TYPE == "otlp":
        if protocol == "grpc":
            exporter = GRPCSpanExporter(
                endpoint=boss_config.OTLP_BASE_ENDPOINT,
                # Header field names must consist of lowercase letters, check RFC7540
                headers=(("authorization", f"Bearer {boss_config.OTLP_API_KEY}"),),
                insecure=True,
            )
            ...
            exporter = HTTPSpanExporter(
                endpoint=boss_config.OTLP_BASE_ENDPOINT + "/v1/traces",
                headers={"Authorization": f"Bearer {boss_config.OTLP_API_KEY}"},
            )
            metric_exporter = HTTPMetricExporter(
                endpoint=boss_config.OTLP_BASE_ENDPOINT + "/v1/metrics",
                headers={"Authorization": f"Bearer {boss_config.OTLP_API_KEY}"},
            )
    else:
    ...
    provider.add_span_processor(
        BatchSpanProcessor(
            exporter,
            max_queue_size=boss_config.OTEL_MAX_QUEUE_SIZE,
            schedule_delay_millis=boss_config.OTEL_BATCH_EXPORT_SCHEDULE_DELAY,
            max_export_batch_size=boss_config.OTEL_MAX_EXPORT_BATCH_SIZE,
            export_timeout_millis=boss_config.OTEL_BATCH_EXPORT_TIMEOUT,
        )
    )
    reader = PeriodicExportingMetricReader(
        metric_exporter,
        export_interval_millis=boss_config.OTEL_METRIC_EXPORT_INTERVAL,
        export_timeout_millis=boss_config.OTEL_METRIC_EXPORT_TIMEOUT,
    )
...
def is_enabled():
    return boss_config.ENABLE_OTEL
