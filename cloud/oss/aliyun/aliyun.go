...
import (
	...
	bossoss "github.com/boss-net/api/cloud/oss"
)
...
func NewAliyunOSSStorage(args bossoss.OSSArgs) (bossoss.OSS, error) {
	var err error
	if args.AliyunOSS == nil {
		return nil, bossoss.ErrArgumentInvalid.WithDetail("can't find Aiyun OSS argument in OSSArgs")
	}
	...
	if err != nil {
		return nil, bossoss.ErrProviderInit.WithError(err).WithDetail("failed to create Aliyun OSS client")
	}

	// get specified bucket
	bucket, err := client.Bucket(bucketName)
	if err != nil {
		return nil, bossoss.ErrProviderInit.WithError(err).WithDetail(fmt.Sprintf("failed to get bucket %s", bucketName))
	}
...
func (s *AliyunOSSStorage) State(key string) (bossoss.OSSState, error) {
	fullPath := s.fullPath(key)
	meta, err := s.bucket.GetObjectMeta(fullPath)
	if err != nil {
		return bossoss.OSSState{}, err
	}
	...
	return bossoss.OSSState{
		Size:         size,
		LastModified: lastModified,
	}, nil
}

func (s *AliyunOSSStorage) List(prefix string) ([]bossoss.OSSPath, error) {
	...
	for {
		...
		for _, object := range lsRes.Objects {
			...
			keys = append(keys, bossoss.OSSPath{
				Path:  key,
				IsDir: false,
			})
		}
...
func (s *AliyunOSSStorage) Type() string {
	return bossoss.OSS_TYPE_ALIYUN_OSS
}
