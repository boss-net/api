...
class PluginInstallationPermissionModel(BaseModel):
    ...
    #   official_only: allow only Boss official plugins
...
class SystemFeatureModel(BaseModel):
    ...
    max_plugin_package_size: int = boss_config.PLUGIN_MAX_PACKAGE_SIZE
...
class FeatureService:
    ...
    def get_features(cls, tenant_id: str) -> FeatureModel:
        ...
        if boss_config.BILLING_ENABLED and tenant_id:
            cls._fulfill_params_from_billing_api(features, tenant_id)

        if boss_config.ENTERPRISE_ENABLED:
    ...
    def get_system_features(cls) -> SystemFeatureModel:
        ...
        if boss_config.MARKETPLACE_ENABLED:
    ...
    def _fulfill_system_params_from_env(cls, system_features: SystemFeatureModel):
        system_features.enable_email_code_login = boss_config.ENABLE_EMAIL_CODE_LOGIN
        system_features.enable_email_password_login = boss_config.ENABLE_EMAIL_PASSWORD_LOGIN
        system_features.enable_social_oauth_login = boss_config.ENABLE_SOCIAL_OAUTH_LOGIN
        system_features.is_allow_register = boss_config.ALLOW_REGISTER
        system_features.is_allow_create_workspace = boss_config.ALLOW_CREATE_WORKSPACE
        system_features.is_email_setup = boss_config.MAIL_TYPE is not None and boss_config.MAIL_TYPE != ""

    @classmethod
    def _fulfill_params_from_env(cls, features: FeatureModel):
        features.can_replace_logo = boss_config.CAN_REPLACE_LOGO
        features.model_load_balancing_enabled = boss_config.MODEL_LB_ENABLED
        features.dataset_operator_enabled = boss_config.DATASET_OPERATOR_ENABLED
        features.education.enabled = boss_config.EDUCATION_ENABLED
